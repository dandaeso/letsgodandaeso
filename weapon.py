...
import util as u
# weapon클래스가 가져야되는것
# cooltime,몇번 반복할지,데미지,angle,position
# 생성자로 가져야되는것
# 없음?
...
class Buff_data:
    ...
    def get_data(self):
        try:
            return float(u.get_data(self.name,self.level))
        except:
            return float(u.get_data(self.name,self.level)[0]),float(u.get_data(self.name,self.level)[1])
class Weapon_Data:
    ...
    def get_cooltime_limit(self):
        value=u.get_data(self.name,self.level)
...
class Dagger_Data(Weapon_Data):
    def __init__(self, character, screen_size, name):
        ...
        self.im=pygame.image.load("assets/weapon/dagger.png")
...
class Staff_Data(Weapon_Data):
    def __init__(self, character, screen_size, name):
        super().__init__( character, screen_size, name)
        self.im=pygame.image.load("assets/weapon/staff.png")
    def get_data(self,random_angle,repeat,m_x,m_y):
...
class Star_Data(Weapon_Data):
    def __init__(self, character, screen_size, name):
        super().__init__(character, screen_size, name)
        self.im=pygame.image.load("assets/weapon/star.png")
    def get_data(self, random_angle, max_repeat, repeat, m_x, m_y):
        # max_repeat에 따라 각도를 일정하게 나눔
        angle_offset = (2 * math.pi) / max_repeat  # max_repeat에 맞춰 각도를 나눔
...
class Missile_Data(Weapon_Data):
    def __init__(self, character, screen_size, name):
        super().__init__(character, screen_size, name)
        missile_im=pygame.image.load("assets/weapon/missile.png")
...
class Mine_Data(Weapon_Data):
    def __init__(self, character, screen_size, name):
        super().__init__(character, screen_size, name)
        im=pygame.image.load("assets/weapon/mine.png")
...
class Sword_Data(Weapon_Data):
    def __init__(self, character, screen_size, name):
        super().__init__(character, screen_size, name)
        im=pygame.image.load("assets/weapon/sword.png")
        sheet = spritesheet.SpriteSheet(im)
        self.im=[]
        for i in range(2):
            for j in range(8):
...
class Weapon_effect:
    def __init__(self, im, hitbox, x, y, target_pos_x, target_pos_y,damage):
        ...
        self.hitbox.left = x
        self.hitbox.left = y  # 원본 rect를 변경하지 않도록 복사
        self.x = x
        self.y = y
        self.damage = damage
    ...
    def screen_move_check(self,key_pressed,deltatime,speed):
        ...
        if key_pressed[pygame.K_UP]or key_pressed[pygame.K_w]:
            ...
            self.screen_move(0,-speed,deltatime)
    def extinction_condition(sele,bg_rect,c_rect):
        pass
...
class Move_weapon(Weapon_effect):
    def __init__(self, im, hitbox, x, y, target_pos_x, target_pos_y,damage):
        ...
        try:
            self.rotate_im = pygame.transform.rotate(self.im, -self.angle)  # 이미지를 반시계 방향으로 회전
        except:
...
class Missile(Move_weapon):
    def __init__(self, frame, hitbox, x, y, target_pos_x, target_pos_y,damage):
        ...
        self.hitbox.left = self.x
        self.hitbox.top = self.y
        self.exploded = False  # 폭발 상태를 추적하는 플래그
    def move(self, deltatime,c_x,c_y):
...
class Dagger(Move_weapon):
    def __init__(self,im, hitbox, x, y, target_pos_x, target_pos_y, distance,damage):
        ...
        self.speed = 2
        self.original_im = self.rotate_im  # 원래 이미지 저장
        self.return_t=False
    def extinction_condition(self,bg_rect,c_rect):
    ...
    def move(self, deltatime,c_x,c_y):
        ...
        if distance_to_center < self.distance and self.return_t==False:
            ...
            if self.pause <= 0:
                ...
                if self.pause>=30:
                    self.rotate_im = pygame.transform.rotate(self.original_im, self.pause*12)
                else:
...
class Mine(Weapon_effect):
    ...
    def update(self):
        if self.exploded:
            ...
            self.hitbox.left = self.x+self.hitbox.width*2.2
            self.hitbox.top = self.y+self.hitbox.height*2.2
    def drawing(self, background):
        background.blit(self.im, (self.x, self.y))
        #pygame.draw.rect(background, (0, 0, 255), self.hitbox, 5)

    def extinction_condition(self, bg_rect, c_rect):
        if self.exploded and self.updat >= 8:
            return True
        return False
    ...
    def change_frame(self, deltatime, enemy):
        if self.exploded:
            ...
            if self.updat >= len(self.frame):
                self.updat = len(self.frame) - 1  # 마지막 프레임을 유지
            self.im = self.frame[int(self.updat)]
